...
// Quote validation
async function validateQuote(quote, options = {}) {
    const errors = [];
    const { checkUniqueness = false, excludeQuoteId, checkCustomerExists = false, checkVehiclesExist = false, } = options;
    ...
    if (!isNonEmptyString(quote.number)) {
        ...
        try {
            const allQuotes = await (0, storage_1.getAllQuotes)();
            const existingQuote = allQuotes.find((q) => q.number === quote.number && q.id !== excludeQuoteId);
            if (existingQuote) {
    ...
    if (!quote.customer || !quote.customer.id) {
        ...
        try {
            const customers = await (0, storage_1.getAllCustomers)();
...
// Invoice validation
async function validateInvoice(invoice, options = {}) {
    const errors = [];
    const { checkUniqueness = false, excludeInvoiceId, checkCustomerExists = false, checkQuoteExists = false, checkPOExists = false, } = options;
    ...
    if (!isNonEmptyString(invoice.number)) {
        ...
        try {
            const allInvoices = await (0, storage_1.getAllInvoices)();
            const existingInvoice = allInvoices.find((inv) => inv.number === invoice.number && inv.id !== excludeInvoiceId);
            if (existingInvoice) {
    ...
    if (invoice.quoteId && checkQuoteExists) {
        try {
            const quote = await (0, storage_1.getQuoteById)(invoice.quoteId);
            if (!quote) {
...
function validateQuoteForExport(quote) {
    ...
    if (!quote.items || quote.items.length === 0) {
        ...
        const hasValidItem = quote.items.some((item) => isNonEmptyString(item.vehicleTypeId) && item.unitPrice > 0);
        if (!hasValidItem) {
            errors.push({
                field: 'items',
...
// Simplified validation for export (no async checks)
function validateInvoiceForExport(invoice) {
    const errors = [];
    // Basic required fields
    if (!isNonEmptyString(invoice.number)) {
        errors.push({ field: 'number', message: 'Invoice number is required' });
    ...
    if (!invoice.items || invoice.items.length === 0) {
        ...
        const hasValidItem = invoice.items.some((item) => isNonEmptyString(item.description) && item.unitPrice > 0);
        if (!hasValidItem) {
            errors.push({
                field: 'items',
