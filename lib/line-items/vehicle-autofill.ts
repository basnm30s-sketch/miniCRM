...
export type VehicleAutofillField = 'vehicleNumber' | 'vehicleTypeId'

export interface VehicleAutofillInput {
  vehicles: VehicleLike[]
  field: VehicleAutofillField
  value: string
}

export interface VehicleAutofillPatch {
...
function buildClearPatch(field: VehicleAutofillField, value: string): VehicleAutofillPatch {
  return {
...
function buildOverwritePatch(vehicle: VehicleLike, field: VehicleAutofillField): VehicleAutofillPatch {
  const vehicleNumber = vehicle.vehicleNumber ?? ''
  const vehicleTypeLabel = vehicle.vehicleType || vehicle.vehicleNumber || ''

  const patch: VehicleAutofillPatch = {
 ...
 */
export function computeVehicleAutofillPatch(input: VehicleAutofillInput): VehicleAutofillPatch {
  ...
  const vehicle = vehicles.find((v) =>
    field === 'vehicleNumber' ? v.vehicleNumber === value : v.id === value
  )

  if (!vehicle) {
    // Unknown value — preserve existing manual edits, only update the
    // field the user typed into.
    return { [field]: value } as VehicleAutofillPatch
  }
 ...
 */
export function recomputeLineTotals<
  T extends { quantity?: number | null; unitPrice?: number | null; taxPercent?: number | null }
>(item: T): T & LineItemTotals {
