import {
    computeVehicleAutofillPatch,
    recomputeLineTotals,
    VehicleLike,
} from '../line-items/vehicle-autofill'
...
describe('computeVehicleAutofillPatch', () => {
    describe('selecting a vehicle on an empty line item', () => {
        test('populates description, metadata, and rate from the master record', () => {
            const patch = computeVehicleAutofillPatch({
                vehicles,
                field: 'vehicleNumber',
                value: 'AAA-111',
            })
    ...
    describe('switching vehicles (regression for the reported bug)', () => {
        test('overwrites description when switching from vehicle A to vehicle B', () => {
            const fromA = computeVehicleAutofillPatch({
                vehicles,
                field: 'vehicleNumber',
                value: 'AAA-111',
            })
            ...
            const fromB = computeVehicleAutofillPatch({
                vehicles,
                field: 'vehicleNumber',
                value: 'BBB-222',
            })
...
describe('recomputeLineTotals', () => {
    test('produces correct totals for typical inputs', () => {
        const result = recomputeLineTotals({
            quantity: 3,
            unitPrice: 100,
            taxPercent: 5,
        })
    ...
    test('preserves additional fields on the input item', () => {
        ...
        expect((result as any).id).toBe('line-1')
        expect((result as any).description).toBe('foo')
    })
})
